...
/**
 * Adapter: implements StageDriver by calling the FastAPI simulator's REST endpoints. This is the
 * only class in the codebase that knows the stage is currently "a Python process on localhost:8000"
 * rather than real hardware — swapping to a real stage controller later means writing a new class
...
public class FastApiStageDriver implements StageDriver {
  ...
  public void init() {
    try {
      connect();
      log.info("Successfully connected to FastAPI simulator");
    } catch (Exception e) {
      log.warn("FastAPI simulator unreachable on startup:{}", e.getMessage());
    }
  ...
  public boolean isHealthy() {
    try {
      client.get().uri("/stage/position").retrieve().toBodilessEntity().block(TIMEOUT);
      return true;
    } catch (Exception e) {
      return false;
    }
  ...
  public void moveTo(double x, double y, double z) {
    ...
    try {
      client.post().uri("/stage/move").bodyValue(new MoveRequest(x, y, z)).retrieve()
  ...
  public Position getPosition() {
    try {
      Position position =
          client.get().uri("/stage/position").retrieve().bodyToMono(Position.class).block(TIMEOUT);
      if (position == null) {
        throw new DriverCommunicationException("Simulator returned empty position");
      }
      return position;
    } catch (WebClientResponseException e) {
      throw new DriverCommunicationException("Failed to read stage position: " + e.getStatusCode(),
          e);
    }
  }
}
